...
$eId = $_GET['eId'];
$exam = 'SELECT * FROM m_exam WHERE id='.$_GET['eId'];
...
while ($t = mysql_fetch_assoc($res)) {
    ...
    $sql = 'SELECT * FROM m_log WHERE user_id='.$_SESSION['userid'].' AND exam_id='.$eId.' AND status=1 AND question_id='.$ques['id'];
    $log = mysql_query($sql);
...
$sql = 'UPDATE m_record SET score='.$total.' WHERE exam_id='.$exam['id'].' AND user_id='.$_SESSION['userid'];
mysql_query($sql);
                                            ...
                                            foreach ($data as $i => $u):
                                                ...
                                                if($u['question']['question_type']==3)
                                                {
                                                    ?>
                                                        ...
                                                        <span style="display: inline-block;color: #000088"> Answer </span>:<?php echo $u['answer'];?>
...
if(!empty($u['option']))
{
    ...
    <span>your answer <?php echo $u['my_answer'];?></span>
    <span style="color: #000080">Score</span>
    <span><?php echo $u['score'];?></span>
